...
%build
%{?scl:scl enable %{scl} "}
%{__python} setup-arago-common-base.py build
%{?scl:"}
...
%{?scl:scl enable %{scl} "}
%{__python} setup-arago-common-base.py install -O1 --skip-build --root %{buildroot} --install-scripts %{python_scriptdir} --install-purelib %{python_sitelib} --install-data %{python_sharedir}
%{?scl:"}
