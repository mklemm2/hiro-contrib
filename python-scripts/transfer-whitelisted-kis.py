...
import subprocess as sp, os, time, json, socket, urlparse, pycurl, shlex, sys, xml.etree.ElementTree as et
...
def transfer_var(var_name, source_graph, target_graph):
	...
	try: target_graph.replace('/_variables/', data=var_body)
			...
			if e.http_code==409: print >>sys.stderr, "Variable \"{var_name}\" already exists!".format(var_name=var_name)
...
def transfer_ki(ki, source_graph, target_graph):
	...
	try:
		...
		try:
			...
			if e.http_code==409:
				print >>sys.stderr, "KI with ID {kiid} and title \"{kititle}\" already exists!".format(kiid=new_ki['ogit/_id'], kititle=new_ki['ogit/name'])
		else:
			for retry in range(1,5):
				while 'ogit/Automation/isDeployed' not in target_graph.get('/{kiid}'.format(kiid=new_ki['ogit/_id'])): time.sleep(0.5); continue
				if target_graph.get('/{kiid}'.format(kiid=new_ki['ogit/_id']))['ogit/Automation/isDeployed']: break
				else: target_graph.update('/{kiid}'.format(kiid=new_ki['ogit/_id']), {
					'ogit/Automation/deployToEngine': True, 'ogit/Automation/deployStatus': None, 'ogit/Automation/isDeployed': None})
...
try:
	...
	tee = sp.Popen(["tee", "-a", "/var/log/autopilot/engine/transfer-whitelisted-kis.log"], stdin=sp.PIPE)
	...
	with Tunnel(config['SourceEnvironment']['EngineHost'], usr=config['SourceEnvironment']['SSHUser'],
		...
		try:
			...
			sys.exit(5)
		try:
			print >>sys.stderr, "{t} Fetching list of already deployed KIs:".format(
				t=time.strftime("[%Y-%m-%d %H:%M:S]"))
			deployed_ki_ids = [item['ogit/_id'] for item in target_graph.query("ogit\/_type:\"ogit/Automation/KnowledgeItem\"", fields=['ogit/_id'])]
			for ki in deployed_ki_ids:
				print >>sys.stderr, ki
			print >>sys.stderr, ""
		except RESTError as e:
			print >>sys.stderr, "{t} Error connecting to {url}: {msg}".format(
				...
				msg=e)
			sys.exit(5)
		query_string = "+ogit\/_type:\"ogit/Automation/KnowledgeItem\" -ogit\/_id:" + "(\"" + "\" \"".join(deployed_ki_ids) + "\")"
		print >>sys.stderr, "{t} Fetching whitelist:".format(
			t=time.strftime("[%Y-%m-%d %H:%M:S]"))
		whitelist = Whitelist(path=config['SourceEnvironment']['DryRunWhitelistPath'], host=config['SourceEnvironment']['EngineHost'], usr=config['SourceEnvironment']['SSHUser'], opts=shlex.split(config['SourceEnvironment']['SSHOptions']))
		for entry in whitelist.kiids:
			print >>sys.stderr, entry
		print >>sys.stderr, ""
		try:
			for ki in source_graph.query(query_string):
				if ki['ogit/_id'] not in whitelist.kiids and has_action(ki):
					print >>sys.stderr, "{t} KI {ki} has actions but is not in whitelist, skipping!".format(
